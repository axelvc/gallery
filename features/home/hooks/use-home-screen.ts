...
} from "@/features/home/utils/persistence";
import {
	isAppOwnedLocalPhotoUri,
	mergePhotos,
	persistPickedPhotoAssetUri,
	splitPhotosByLock,
} from "@/features/home/utils/photos";
...
export function useHomeScreen() {
	...
	const sortable = useSortableList({
		...
		autoScrollBackThreshold: 0.05,
		autoScrollForwardThreshold: 0.95,
	...
	const resetProfileToBlank = useCallback(
		(nextUsername: string) => {
		...
		async (requestedUsername: string) => {
			...
			if (!normalizedUsername || profileLoadLockRef.current) {
				if (!normalizedUsername) {
					Alert.alert(
						t("home.alerts.usernameNeededTitle"),
						t("home.alerts.usernameNeededMessage"),
					);
				}
			...
			try {
				...
				if (error instanceof Error) {
					if (error.message === "PROFILE_PRIVATE") {
						Alert.alert(
							t("home.alerts.privateProfileTitle"),
							t("home.alerts.privateProfileMessage"),
						);
					} else if (error.message === "PROFILE_BLOCKED") {
						Alert.alert(
							t("home.alerts.profileBlockedTitle"),
							t("home.alerts.profileBlockedMessage"),
						);
					} else if (error.message === "PROFILE_NOT_FOUND") {
						Alert.alert(
							t("home.alerts.profileNotFoundTitle"),
							t("home.alerts.profileNotFoundMessage"),
						);
						...
						Alert.alert(
							t("home.alerts.proxyNotConfiguredTitle"),
							t("home.alerts.proxyNotConfiguredMessage"),
						);
					} else {
						Alert.alert(
							t("home.alerts.profileUnavailableTitle"),
							t("home.alerts.profileUnavailableMessage"),
						);
					}
				}
			} finally {
		...
		try {
			if (Platform.OS !== "web") {
				...
				if (!permission.granted) {
					Alert.alert(
						t("home.alerts.photoAccessNeededTitle"),
						t("home.alerts.photoAccessNeededMessage"),
					);
					return;
				}
			}
			...
			for (const entry of picked) {
				void (async () => {
					try {
						const persistedUri = await persistPickedPhotoAssetUri(
							entry.asset,
							entry.id,
						);
			...
			Alert.alert(
				t("home.alerts.unableToOpenGalleryTitle"),
				t("home.alerts.unableToOpenGalleryMessage"),
			);
		} finally {
			setIsPicking(false);
		}
	...
	const removePhoto = useCallback((photoId: string) => {
		setPhotos((current) =>
			current.filter((photo) => photo.id !== photoId || photo.locked),
		);
	}, []);
